...
func TestRequestLineParse(t *testing.T) {
	...
	require.Error(t, err)
	assert.ErrorIs(t, err, ERROR_MALFORMED_REQUEST)

	// // Test: Invalid HTTP Version ( not 1.1)
	// _, err = RequestFromReader(strings.NewReader("GET / HTTP/2.0\r\n\r\n"))
	// require.Error(t, err)
	// assert.ErrorIs(t, err, ERROR_MALFORMED_REQUEST)

	// // Test: Invalid HTTP version Format
	// _, err = RequestFromReader(strings.NewReader("GET / HTTPS/1.1\r\n\r\n"))
	// require.Error(t, err)
	// assert.ErrorIs(t, err, ERROR_MALFORMED_REQUEST)
	...
	// Test : GOOD Request line with path and higher numBytesPerRead
	reader = &chunkReader{
		data:            "GET /coffee HTTP/1.1\r\nHost: localhost:42069\r\nUser-Agent: curl/7.81.0\r\nAccept: */*\r\n\r\n",
		numBytesPerRead: 128,
	}
